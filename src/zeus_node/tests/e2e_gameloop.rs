...
async fn make_node(cell: zeus_node::cell::Cell, peers: Vec<std::net::SocketAddr>) -> GameLoop<TestWorld> {
    let config = ZeusConfig {
        bind_addr: "127.0.0.1:0".parse().unwrap(),
        seed_addrs: peers,
        boundary: cell.x_max,
...
fn assert_single_ownership(nodes: &[GameLoop<TestWorld>], label: &str) {
    use zeus_node::entity_manager::AuthorityState;
...
fn total_local_count(nodes: &[GameLoop<TestWorld>]) -> usize {
    use zeus_node::entity_manager::AuthorityState;
    nodes.iter()
